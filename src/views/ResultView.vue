...
const passCount = computed(
  () => store.results?.filter((r: any) => r.is_success === true).length ?? 0,
);

const failCount = computed(
  () => store.results?.filter((r: any) => r.is_success === false).length ?? 0,
);
...
<template>
  ...
  >
    <div class="p-6 max-w-4xl mx-auto">
      ...
      >
        <div
          class="w-20 h-20 rounded-full bg-blue-100 flex items-center justify-center overflow-hidden"
        >
          <img
      ...
      <div class="bg-[#FFF9D2] border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        <!-- Table Header -->
        <div class="grid grid-cols-[2fr_1.6fr_1fr_1fr_100px] px-4 py-2.5 border-b border-gray-200">
          ...
          <div class="text-md font-semibold text-gray-500 text-center">درجة الترم</div>
          ...
          <div class="flex items-center gap-2 px-2">
            <div class="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                "
                :style="{
                  width: scorePercent(result.points, result.max_grade) + '%',
                }"
              />
            </div>

            <span class="text-lg font-semibold text-gray-700 min-w-20 text-left whitespace-nowrap">
