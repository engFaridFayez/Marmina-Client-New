...
onMounted(async () => {
  ...
  if (firstExam) {
    ...
    await resultStore.getComponentExams({ exam: firstExam.id });
  }
});

const onExamChange = async (examId: number) => {
  selectedExamId.value = examId;
  await resultStore.getComponentExams({ exam: examId });
};

const goToCreate = () => {
  if (!selectedExamId.value) return;
  router.push(`/admin/results/component-exams/new?exam=${selectedExamId.value}`);
};

const goToEdit = (componentExamId: number) => {
  router.push(`/admin/results/component-exams/${componentExamId}/edit`);
};

const removeComponentExam = async (componentExamId: number) => {
  if (!confirm("هل أنت متأكد من حذف درجة هذا الجزء؟")) return;

  deletingId.value = componentExamId;

  try {
    await resultStore.deleteComponentExam(componentExamId);
  } finally {
...
<template>
  <div dir="rtl" class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-[#232A7E]">درجات الأجزاء لكل امتحان</h1>
        <p class="text-gray-500 mt-2">تحديد الدرجة القصوى ودرجة النجاح لكل جزء في كل امتحان</p>
      </div>
    ...
    <!-- Empty -->
    <div
      v-else-if="resultStore.componentExams.length === 0"
      class="bg-white rounded-2xl p-10 text-center text-gray-400"
    >
      <span v-if="selectedExam">لا توجد درجات أجزاء مسجلة لامتحان {{ selectedExam.name }} بعد</span>
      <span v-else>اختر امتحانًا لعرض درجات الأجزاء</span>
    </div>

    <!-- List -->
    <div v-else class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-3">
      <div
        v-for="componentExam in resultStore.componentExams"
        :key="componentExam.id"
        class="flex items-center justify-between border border-gray-100 rounded-xl p-4"
      >
        <div>
          <h4 class="font-bold text-gray-800">
            {{ componentExam.subject_name }}
            <span class="text-sm font-normal text-gray-400">
              ({{ componentExam.component_name }})
            </span>
          </h4>
          <p class="text-sm text-gray-400 mt-1">{{ componentExam.exam_name }}</p>
        </div>
        ...
        <div class="flex items-center gap-6">
          <div class="text-sm text-left">
            <p class="text-gray-500">
              الدرجة القصوى:
              <span class="font-bold text-[#232A7E]">{{ componentExam.max_grade }}</span>
            </p>
            <p class="text-gray-500 mt-1">
              درجة النجاح:
              <span class="font-bold text-green-600">
                {{ componentExam.success_grade ?? "—" }}
              </span>
            </p>
          </div>
          ...
          <div class="flex gap-2">
            <button
              @click="goToEdit(componentExam.id)"
            ...
            <button
              @click="removeComponentExam(componentExam.id)"
              :disabled="deletingId === componentExam.id"
